# 3. Criando uma Lista de Produtos
# Muitas vezes as listas de produtos, de clientes, de funcionários, etc.,
# que temos nas empresas não são 100% confiáveis.
#
# Isso significa que, na prática, caímos em muitos casos de ter uma lista de produtos, mas quando olhamos, por exemplo,
# uma base com a posição em estoque de cada produto, surge nessa "base" produtos que não existiam na nossa lista
# inicial.
#
...
#
# Vamos criar uma function que pega todas essas listas e cria, a partir dela, uma lista de produtos. Para
#  saber todos os produtos que temos na empresa. Repare que a function pode receber 1, 2, 3 ou quantas listas de
# estoque forem necessárias.
#
...
def definir_produtos_em_estoque(*args):
    produtos = []
    for lista in args:
        for produto, unidades in lista:
            produtos.append(produto)
    return set(produtos)


produtos_estoque = definir_produtos_em_estoque(estoque_fim_jan, estoque_fim_fev, estoque_fim_mar, estoque_fim_abr)

lista_produtos_estoque = list(produtos_estoque)

print(lista_produtos_estoque)
